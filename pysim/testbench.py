...
async def train(dut, ref_model, console):
    ...
    dut.state.value = 0
    for i in range(ref_model.cfg.SET_SIZE):
        for size in range(ref_model.cfg.SMP_SIZE):
            dut.smp_clr.value = 0
            dut.smp_en.value = 1
            dut.im_value.value = ref_model.feat(i, size)
            dut.im_pos.value = ref_model.pos(i, size)
            dut.label.value = ref_model.label
...
async def predict(dut, ref_model, num, console):
    ref_model.train()
    dut.state.value = 1
